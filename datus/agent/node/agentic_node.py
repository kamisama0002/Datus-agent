...
class AgenticNode(Node):
    ...
    def _render_orchestrator_context_part(context: Optional[Dict[str, Any]]) -> str:
        ...
        return (
            ...
            "carry into the current turn. For a single scoped data query or follow-up, use direct metadata and query "
            "tools and do not delegate merely to repeat discovery. Delegate to a task or subagent only when the "
            "current request contains multiple independent workstreams that genuinely benefit from separation. "
            "Historical messages, summaries, sample values, "
